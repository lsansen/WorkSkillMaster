...
const MeetingNotesExpert: React.FC = () => {
  const skillPackage = {
    ...
    parameters: [
      ...
      },
      {
        ...
        type: 'file',
        label: '会议资料（文件上传）',
        description: '上传会议相关文件（支持TXT、MD格式）',
        required: false
      },
      {
        id: 'notesStyle',
  ...
  const handleGenerate = async (params: any): Promise<string> => {
    ...
    let referenceText = '';
    if (params.audioFile || params.meetingFile) {
      referenceText = '（参考了上传的会议资料）';
    }
...
（注：这是由AI助手生成的${notesStyleText}会议纪要${referenceText}，建议根据实际情况进行调整和完善。）`;
  };
